...
for i in $(cat /etc/puppetlabs/code/modules/puppetfiles/files/hosts |grep node[[:digit:]] |awk {'print $1'}); do echo service $i:8080 max_fails=0\; >> /etc/puppetlabs/code/modules/puppetfiles/files/upstream.conf; done
